...
class EvaluacionController extends AppControllerBase{
    ...
    public async getController(req:Request, res:Response): Promise<void>{
        try {
            const result = await EvaluacionService.getService();
            res.json(result);
        } catch (error) {
    ...
    public async putController(req:Request, res:Response): Promise<void> {
        try {
            ...
            const result = await EvaluacionService.putService(req.body, req.params.id);
    
            res.status(200).json({ message: "Evaluacion actualizada correctamente" , data: result });
        }
        catch (error) {
